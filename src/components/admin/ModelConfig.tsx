...
export const ModelConfig: React.FC = () => {
    ...
    if (loading) return <div className="flex items-center gap-2 text-neutral-400 dark:text-neutral-500 text-sm py-8"><Loader2 size={16} className="animate-spin" /> 加载中…</div>;

    return (
        <div className="max-w-5xl">
            {/* ---------- Providers ---------- */}
            <div className="flex items-center justify-between mb-3">
                <h2 className="text-lg font-semibold text-neutral-900 dark:text-white flex items-center gap-2"><Plug size={18} /> 接入点</h2>
            ...
            <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 overflow-hidden mb-8">
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {providers.map(p => (
                            <tr key={p.id} className="border-t border-neutral-200 dark:border-neutral-800 text-neutral-800 dark:text-neutral-200">
                                <td className="px-4 py-2.5 font-medium">{p.name}</td>
                                <td className="px-4 py-2.5"><span className="text-[11px] px-1.5 py-0.5 rounded bg-neutral-200 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300">{p.kind === 'fp' ? 'Flow (fp)' : 'OpenAI 兼容'}</span></td>
                                <td className="px-4 py-2.5 text-[12px] text-neutral-500 dark:text-neutral-400 font-mono truncate max-w-[260px]">{p.baseUrl || '—'}</td>
                                ...
                                <td className="px-4 py-2.5">
                                    <div className="flex items-center justify-end gap-1.5">
                                        <Tip label="测试连通"><button onClick={() => testProvider(p)} disabled={testingId === p.id} className="p-1.5 rounded hover:bg-neutral-200 dark:hover:bg-neutral-700 text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white disabled:opacity-50">{testingId === p.id ? <Loader2 size={15} className="animate-spin" /> : <Plug size={15} />}</button></Tip>
                                        <Tip label="编辑"><button onClick={() => setProviderModal({ open: true, provider: p })} className="p-1.5 rounded hover:bg-neutral-200 dark:hover:bg-neutral-700 text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white"><Pencil size={15} /></button></Tip>
                                        <Tip label="删除"><button onClick={() => askDeleteProvider(p)} className="p-1.5 rounded hover:bg-red-500/20 text-neutral-500 dark:text-neutral-400 hover:text-red-400"><Trash2 size={15} /></button></Tip>
                                    </div>
                                </td>
                            </tr>
                        ))}
                        {!providers.length && <tr><td colSpan={5} className="px-4 py-6 text-center text-neutral-400 dark:text-neutral-500 text-sm">暂无接入点</td></tr>}
                    </tbody>
                </table>
            </div>
            ...
            {/* ---------- Models by category ---------- */}
            <div className="flex items-center justify-between mb-3">
                <h2 className="text-lg font-semibold text-neutral-900 dark:text-white flex items-center gap-2"><ListChecks size={18} /> 模型清单</h2>
            </div>
                    ...
                    return (
                        <div key={cat}>
                            <div className="flex items-center justify-between mb-2">
                                <h3 className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">{CATEGORY_LABEL[cat]}<span className="ml-2 text-[11px] text-neutral-400 dark:text-neutral-500">{list.length} 个</span></h3>
                            ...
                            <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 overflow-hidden">
                                <table className="w-full text-sm table-fixed">
                                    ...
                                    <tbody>
                                        {list.map(m => (
                                            <tr key={m.id} className={`border-b border-neutral-200 dark:border-neutral-800 last:border-0 ${m.enabled ? 'text-neutral-800 dark:text-neutral-200' : 'text-neutral-400 dark:text-neutral-600'}`}>
                                                <td className="pl-4 pr-1 py-2.5">
                                                    <Tip label={m.isDefault ? '默认模型' : '设为默认'}>
                                                        <button onClick={() => setDefault(m)} className={m.isDefault ? 'text-amber-400' : 'text-neutral-400 dark:text-neutral-600 hover:text-neutral-700 dark:hover:text-neutral-300'}><Star size={15} fill={m.isDefault ? 'currentColor' : 'none'} /></button>
                                                    </Tip>
                                                </td>
                                                <td className="px-2 py-2.5">
                                                    ...
                                                    <div className="text-[11px] text-neutral-400 dark:text-neutral-500 font-mono truncate">{m.modelId}</div>
                                                </td>
                                                <td className="px-2 py-2.5 text-[12px] text-neutral-500 dark:text-neutral-400 truncate">{providerName(m.providerId)}</td>
                                                <td className="px-2 py-2.5">
                                                    <div className="flex items-center justify-end gap-1.5">
                                                        <Tip label={m.enabled ? '已启用（点击停用）' : '已停用（点击启用）'}>
                                                            <button onClick={() => toggleEnabled(m)} className={m.enabled ? 'text-green-400' : 'text-neutral-400 dark:text-neutral-600 hover:text-neutral-700 dark:hover:text-neutral-300'}>{m.enabled ? <CheckCircle2 size={16} /> : <Circle size={16} />}</button>
                                                        </Tip>
                                                        <Tip label="编辑"><button onClick={() => setModelModal({ open: true, model: m })} className="p-1.5 rounded hover:bg-neutral-200 dark:hover:bg-neutral-700 text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white"><Pencil size={15} /></button></Tip>
                                                        <Tip label="删除"><button onClick={() => askDeleteModel(m)} className="p-1.5 rounded hover:bg-red-500/20 text-neutral-500 dark:text-neutral-400 hover:text-red-400"><Trash2 size={15} /></button></Tip>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                                        {!list.length && <tr><td colSpan={4} className="px-4 py-4 text-center text-neutral-400 dark:text-neutral-500 text-xs">暂无模型</td></tr>}
                                    </tbody>
                                </table>
                            </div>
                        </div>
