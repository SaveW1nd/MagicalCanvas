    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[130]" onClick={close}>
            <form onClick={e => e.stopPropagation()} onSubmit={submit}
                className="w-full max-w-sm bg-white dark:bg-[#1a1a1a] border border-neutral-300 dark:border-neutral-700 rounded-2xl shadow-2xl p-6 flex flex-col gap-3">
                <div className="flex items-center justify-between">
                    <h3 className="text-base font-semibold text-neutral-900 dark:text-white">设置「{username}」的密码</h3>
                ...
                <input type="text" value={pw} onChange={e => setPw(e.target.value)} autoFocus placeholder="留空 = 默认 12345678"
                    className="bg-white dark:bg-neutral-950 border border-neutral-300 dark:border-neutral-700 rounded-lg px-3 py-2 text-sm text-neutral-800 dark:text-neutral-200 outline-none focus:border-blue-500/60" />
