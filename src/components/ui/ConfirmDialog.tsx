...
export const ConfirmDialog: React.FC<{
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[130]" onClick={loading ? undefined : onClose}>
            <div onClick={e => e.stopPropagation()} className="w-full max-w-sm bg-white dark:bg-[#1a1a1a] border border-neutral-200 dark:border-neutral-700 rounded-2xl shadow-2xl p-6">
                <h3 className="text-base font-semibold text-neutral-900 dark:text-white mb-2">{title}</h3>
                {message && <p className="text-sm text-neutral-500 dark:text-neutral-400 mb-5 whitespace-pre-wrap">{message}</p>}
                <div className="flex justify-end gap-2">
                    <button onClick={onClose} disabled={loading}
                        className="px-3 py-1.5 rounded-lg text-sm bg-neutral-200 hover:bg-neutral-300 text-neutral-800 dark:bg-neutral-800 dark:hover:bg-neutral-700 dark:text-white transition-colors disabled:opacity-50">取消</button>
